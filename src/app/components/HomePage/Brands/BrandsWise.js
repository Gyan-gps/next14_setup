        ...
        <div className="brand-two__wrapper">
          <Slider {...settings}>
            {data.map((brand) => {
              return (
                <Link
                  href={`${process.env.PUBLIC_URL}/brands/${brand.main_category_name.trim().replaceAll(" ", "-")}`}
                  key={brand.main_category_name}
                >
                  <img
                    ...
                    // onClick={() => {
                    //   window.location.replace(`/search/${brand.main_category_name}`);
                    // }}
                    loading="lazy"
                  />
                </Link>
